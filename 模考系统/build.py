...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* 补全单词 */
  .cw{font-size:16px;line-height:2.4}
  .cw .pre{font-weight:600}
  .cw input{width:64px;border:0;border-bottom:2px solid var(--accent);background:#fbf1e6;font-size:15px;padding:2px 4px;font-family:inherit;text-align:center}
  .cw input:focus{outline:none;background:#f6e3d0}
  /* 连词成句 */
...
function cwRender(b, body){
  const t = b.task;
  body.innerHTML = `<div class="instr">填出缺失的字母（首句完整，其后每隔一词挖去后半截）。</div><div class="cw" id="cw"></div>`;
  ...
  parts.forEach(p=>{
    const m = p.match(/^\[\[([^|]*)\|([^\]]*)\]\]$/);
    if(m){
      const idx = bi++; const key = b.id+':b'+idx;
      const span = document.createElement('span');
      span.innerHTML = `<span class="pre">${esc(m[1])}</span><input data-k="${key}" size="4" autocomplete="off" spellcheck="false" value="${esc(resp[key]||'')}">`;
      cw.appendChild(span);
    } else if(p){ cw.appendChild(document.createTextNode(p)); }
  });
  cw.querySelectorAll('input').forEach(inp=> inp.oninput = ()=>{ resp[inp.dataset.k]=inp.value.trim(); save(); });
}
