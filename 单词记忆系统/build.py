...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .tag.spk{background:#fdeccd;color:#b5661f}
  .tag.scn{background:#e4f0e6;color:#3d7a4a}
...
def build_index(groups):
    ...
    builtlinks = "".join(
        f'<a href="groups/{esc(g["id"])}.html">{esc(g.get("combo", g["id"]))} {esc(g.get("sound", ""))}</a>'
...
def _is_wordfile(f):
    # 词表组 = 含 "words" 键的 JSON；排除 _ 开头配置(_soundmap) 与非词表文件(如 cloze.json 用于阅读填词)
    if f.name.startswith("_"):
        return False
    try:
        return "words" in json.loads(f.read_text(encoding="utf-8"))
    except Exception:
        return False

def main():
    only = sys.argv[1] if len(sys.argv) > 1 else None
    files = sorted(f for f in DATA.glob("*.json") if _is_wordfile(f))
    if only:
    ...
    all_groups = [json.loads(f.read_text(encoding="utf-8"))
                  for f in sorted(DATA.glob("*.json")) if _is_wordfile(f)]
